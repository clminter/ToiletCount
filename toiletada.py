...
def main():
    ...
    if st.button("Calculate"):
        ...
        st.sidebar.subheader(f"♿ Wheelchair Seating Requirements: {wheelchair_seating}")

        st.sidebar.subheader("Wheelchair Distribution:")
        for i in range(levels):
            st.sidebar.write(f"Level {i+1}: {wheelchair_distribution[i]}")
